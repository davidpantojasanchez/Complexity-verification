  ...
  if nQuestions <= k {
      ...
      //invariant verification_loop(f, g, P, k, a, b, Q, questionsToVerify, candidates, R)
    {
      ...
      assert verification_loop(f, g, P, k, a, b, Q, questionsToVerify, candidates, R) by {
        reveal verification_loop();
        ghost var aux := R == (R_ && forall candidate:map<Question, Answer> | candidate in (candidates_.Model() - candidates.Model()) ::
        (
          var f' := map person:map<Question, Answer> | person in f.Model().Keys && (forall q:Question | q in questionsToVerify.Model() :: person[q] == candidate[q]) :: f.Model()[person];
          var g' := map person:map<Question, Answer> | person in g.Model().Keys && (forall q:Question | q in questionsToVerify.Model() :: person[q] == candidate[q]) :: g.Model()[person];
          okFitness(f') && okPrivate(g', P.Model(), a, b, Q.Model())
        )) by { reveal problem_requirements(); }
        assert aux
        by {
          reveal problem_requirements();
        ...
        assert verification_loop(f, g, P, k, a, b, Q, questionsToVerify, candidates, R) == aux by {
          reveal verification_loop(f, g, P, k, a, b, Q, questionsToVerify, candidates, R);
          assert aux == (R == (R_ && forall candidate:map<Question, Answer> | candidate in (candidates_.Model() - candidates.Model()) ::
          (
          var f' := map person:map<Question, Answer> | person in f.Model().Keys && (forall q:Question | q in questionsToVerify.Model() :: person[q] == candidate[q]) :: f.Model()[person];
          var g' := map person:map<Question, Answer> | person in g.Model().Keys && (forall q:Question | q in questionsToVerify.Model() :: person[q] == candidate[q]) :: g.Model()[person];
          okFitness(f') && okPrivate(g', P.Model(), a, b, Q.Model())
      ...
      assert counter_loop(f, g, P, i, counter_in, counter);
      assume false;
    }
...

{
    ...
    by {
      ...
      assert forall q | q in questionsToVerify.Model() :: q in candidate.Model().Keys by { reveal problem_requirements(); }
      assert forall person:map<Question, Answer> | person in f.Model().Keys :: person.Keys == Q.Model() by { reveal problem_requirements(); }
      assert forall person:map<Question, Answer> | person in g.Model().Keys :: person.Keys == Q.Model() by { reveal problem_requirements(); }
      ...
      by {
        ...
        /* está puesto más arriba pq también se usa fuera del {}
        assert forall q | q in questionsToVerify.Model() :: q in candidate.Model().Keys by { reveal problem_requirements(); }
        assert forall person:map<Question, Answer> | person in f.Model().Keys :: person.Keys == Q.Model() by { reveal problem_requirements(); }
        assert forall person:map<Question, Answer> | person in g.Model().Keys :: person.Keys == Q.Model() by { reveal problem_requirements(); }
        */
